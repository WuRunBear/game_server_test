...
- [x] Lane A 完成（A1-A4；新增 tile-units 10 用例；slice5.test.ts 断言随 server.json 迁移同步更新 304/32）
- [x] Lane B 完成（B1-B4，8 文件；behaviors 6 节点均原已显式传速，按 0.16 公式换算无遗漏，框架 px 默认值不再被触发）
- [x] §6 验证：vitest 56 文件 / 687 测试全绿；`pnpm tools validate` ✓（5 图管道、33 原型，*Tiles 经归一化端到端生效）；boot 冒烟 ✓（端口起服、离线补差 226k ticks、常驻房间创建）
- [ ] 观感确认（96s 横穿 / 0.5 格玩家）——待人工进游戏
- [ ] 第二批任务已开
