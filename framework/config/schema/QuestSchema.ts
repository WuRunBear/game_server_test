 ...
 *
 * 任务目标为「收集 / 击杀」两类通用形态；完成（submit）效果：消耗任务物品
 * （collect 型）+ 发奖励物品 + 好感增减（好感对象为提交对话的 NPC kind，
 * 由 dialogueSystem 传入）。
 */
...
export const QuestDefinitionSchema = z.object({
  ...
  id: z.string(),
  /**
   * 目标形态：collect 背包持有 itemKind 物品 ≥ goal（进度由 questSystem tick 检查）；
