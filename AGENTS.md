...
- **AoS 组件家族**：`Inventory` / `Kind` / `Needs` / `ResourceNode` / `ItemMeta` / `Intent` / `EntityMap` 都是普通 JS 数组（`[] as T[]`，按 eid 索引），与其余 SoA（bitecs 数值数组）不一致。它们**不是 bitecs 组件**——不能 `addComponent`、不能进 `query`。spawn 经组件注册表的 **AoS 初始化钩子**（`registerAosInitializer`）按 archetype 配置写入；netSync 经 **AoS 同步适配器**（`registerAosSyncAdapter`，按 `tags` 限定查询）展平为 numbers/strings。扩展此类组件时注意访问方式与查询方式。例外注意 `EntityMap`：**无 archetype initializer**（运行时地图 id 无法由 archetype 配置表达，spawn 链直接写入）+ 不注册 aosSyncAdapter——它随实体自动入档（`worldSerializer` AoS 分支），实体缺失条目时由 `entityMapOf(world, eid)` 回退 `world.defaultMapId`。
- **多图基础设施**：`world.map` 是默认图的**弃用别名**（`@deprecated`，恒指向 `world.maps[world.defaultMapId]`）——新代码一律经 `world.maps`（惰性缓存 MapRuntime）/ `world.defaultMapId` / `entityMapOf` 访问，不要新增对 `world.map` 的消费。
- **常驻模拟语义**：`world.activeMaps` 里的地图即使没有任何玩家也照常运行碰撞/刷怪等系统（空图"活着"）；地图经 `ensureMapActive` 惰性构建（`world.maps`）+ 激活（`activeMaps.add` + `spawnInitialNpcs` 布置），不开机预建全部图；新玩家默认出生 `world.defaultMapId`。
